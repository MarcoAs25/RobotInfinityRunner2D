...
public class input : MonoBehaviour
{
    ...
    void Update()
    {
        if (!GameManager.inst.getDeath() && GameManager.inst.getInit())//caso o personagem não tenha morrido e tenha iniciado o jogo
        {
            gameObject.GetComponent<Animator>().SetFloat("velx", 10);// começar a correr
            if (Input.GetKeyDown(KeyCode.S))
            {
                ...
                {
                    gameObject.GetComponent<controller>().Movimento(false, true);//cai no chão mais rápido
                }
            }
            if (Input.GetKeyDown(KeyCode.W) && gameObject.GetComponent<groundcheck>().getEstaNoChao()) //caso aperte o botao de pular e esteja no chao
            {
            ...
            gameObject.GetComponent<Animator>().SetFloat("velx", 0);//caso tenha morrido, ou não tenha iniciado a animação seta a velx para 0
        }
    }
    void Movimentar()
    {
        foreach (Touch t in Input.touches)
        {
            ...
            else if ((t.phase == TouchPhase.Moved)&& !swiped)//verifica se o usuário moveu o dedo pela tela
            {
                float yMoved = initTouch.position.y - t.position.y;//verifica quanto foi movido em y
                float distance = Mathf.Sqrt((yMoved * yMoved)); // calcula a distância (h2 = p2 + b2)

                if (distance > 25f)//caso a distância for maior que 25, consideramos como um comando
                {
